...
public class Constructor {
    ...
    public   Constructor() {
        ...
        System.out.println("NoArgsCnstructor(기본생성자)");
    }
    //매개변수 생성자 # 1
    ...
    /*
        ...
        RequiredArgConstructor(String 매개변수를 필수로 요구하는 생성자) 라는 안내문을
        출력할 수 있도록 작성한 뒤
        ConstructorMain 으로 가서 Constructor4라는 객체명을 지니고, title에 '김사'으로
        이름 붙이고 객체를 생성한 후,
        System.ou.println(constructor4.name)을 입력하여 콘솔에 '김사'를 출력하시오

        지시사항 # 2
        int num, String name을 매개변수로 하는 생성자를 정의하고
        AllArgsConstructor(int, String 전부를 매개변수로 요구하는 생성자) 라는 안내문을
        출력할 수 있도록 작성한 뒤
        ConstructorMain으로 가서 constructor5라는 객체명을 지니고, 5, 김오 라고 값을 대입한
        객체를 생성한 후

        지시사항 # 3
        Car.java 파일 내에 있는 displayInfo() 메서드를 참조하여
        constructor5.idsplayinfo();를 호출 했을 경우
        김오학생의 학번은 5번 입니다.
        를 콘솔에 출력할 수 있도록 Constructor.java

     */

    public Constructor(String title) {
        System.out.println("RequiredArgConstructor(String 매개변수를 필수로 요구하는 생성자)");
        this.name = title; // this.필드명 = 매개변수;
    }
    public Constructor(int num1, String name1) {
        System.out.println("AllArgsConstructor(int,String 전부를 매개변수로 요구하는 생성자)");
        this.num = num1;
        this.name = name1;
    }
    public void displayInfo() {
        System.out.println(name +"학생의 학번은" + num + "번입니다.");
