...
//        int[][] lottoNumSet = new int[round][6];
        ...
        for (int j = 0 ; j < lottoNums.length; j ++) {
            for (int i = 0; i < lottoNums.length; i++) {
                ...
                // 추가하기 위해서입니다.
                /*
                    만약에 반복문이 진행되어서 i 가 3인 상황이라고 가정하겠습니다. 그러면 이전 반복까지 해서
                    0 1 2 번지에 난수가 대입되어있을겁니다.
                    그러면 0 1 2 번지의 각각의 int값이 temp값과 일치하는지를 확인하고, 만약에
                    일치하지 않으면 -> 3번지에 temp값을 대입
                    일치하는게 있으면 -> 다시 난수 생성 후 temp에 값 대입하고 다시 확인
                    하는 과정을 거쳐야만 합니다.
                    이하의 반복문은 3 번지라고 가정했을 때 2 번지까지의 element 값을 확인하는 부분입니다.
                 */
                for (int k = 0; k < i; k++) {
                    if (lottoNums[k] == temp) duplicate = true;
                } // 어쨌든 0번지부터 i-1번지까지 전체 다 확인을 할건데 한 번 true로 바뀌면 중복이 되었겠네요
                // 중복이 되면 우린 뭐해야한다? -> 다시 뽑아서 temp에 저장해야한다. 그리고 또 확인해야한다.
