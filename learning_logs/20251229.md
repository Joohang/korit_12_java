...
1. 정의
    ...
    예를 들어, 
    "자동차"라는 클래스를 생성한다면 자동차의 속성(색상, 속도)과 행위(가속, 정지 등)를 코드로 표현할 수 있음

2. 객체(Object)란?
    클래스를 설계도로 가정했을 때, 실제 사용할 수 있는 자동차(인스턴스)는 객체라고 함.

    Object와 Instance는 우리나라에서 거의 동일한 용어로 사용됩니다.
    ...
    비유로 이해하기
        ...
        3. "홍길동"은 "사람"이라는 특정 클래스의 '인스턴스'임, '아반떼'는 아님

# 생성자(Constructor)

1. 정의 : 객체가 생성될 때 호출되는 특별한 '메서드'

    예시

    클래스명 객체명 = new 클래스명 ();
    ClassExample example = new ClassExample();
    ->ClassExample(); 가 생성자에 해당됩니다.

2. 특징 : 
    - 클래스의 이름과 생성자의 이름은 동일하다. (다른 메서드들과 달리 Pascal Case로 작성)
    method 명은 동사이지만 생성자의 이름은 클래스명과 동일하기 때문에 명사
    - return 값이 없다. -> call1(), call2() 유형으로 생각할수 있음.
    - 객체 초기화 : 생성자는 객체의 필드 '초기화'를 담당한다,

3. 정의 방식
    1. 기본 생성자 : 매개변수가 없는 생성자
        (근데 얘는 클래스를 만들때 default로 생성되기 때문에 정의하지 않아도 될 때가 있다.)
